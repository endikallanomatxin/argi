                ...
                ..ok ~ first_payload {
                    ...
                    match second_result {
                        ...
                        ..ok ~ second_payload {
                            ...
                            trusted_opaque_store_owned_in#(.t: Borrowing, .storage_type: Allocation)(.storage = $&container.first, .destination = first_slot, .source = ~first_value)
                            trusted_opaque_store_owned_in#(.t: Borrowing, .storage_type: Allocation)(.storage = $&container.second, .destination = second_slot, .source = ~second_value)
                            trusted_opaque_drop_owned(.slot = first_slot)
                            trusted_opaque_drop_owned(.slot = second_slot)
                            trusted_opaque_release_all(.storage = $&container.first)
                            deinit(.self = $&target)
                            status_code = 0
                        }
                    }
