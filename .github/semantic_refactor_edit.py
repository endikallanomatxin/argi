...
replacement = r'''    fn inferInitializerReachBindings(
        self: *Resolver,
...
'''
new = r'''        const destination_ok = try generic_functions.inferInputType(
            candidate_index,
            storage.fields.items[shape.fields.start].ty,
            destination_pointer,
            bindings,
...
old = '''                const actual = self.graph.nodes.items[@intFromEnum(value.value)].ty orelse break;
                ...
                break;
'''
new = r'''                const actual = self.graph.nodes.items[@intFromEnum(value.value)].ty orelse break;
...
'''
new = r'''        const input_ty = generics.instantiateParameterizedType(candidate_index, parameterized.input, &bindings, null) catch |err| {
