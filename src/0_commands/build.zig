...
fn printCompilerStats(
    ...
    std.debug.print("  module semantizing:    {d:.3} ms\n", .{@as(f64, @floatFromInt(pipeline.module_semantizing_ns)) / 1_000_000.0});
    std.debug.print("  global artifact merge: {d:.3} ms\n", .{@as(f64, @floatFromInt(pipeline.global_merge_ns)) / 1_000_000.0});
    ...
    std.debug.print("  ModuleSemanticGraph total: {d}\n", .{pipeline.moduleSemanticStorageBytes()});
