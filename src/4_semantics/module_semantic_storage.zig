...
pub const Storage = struct {
    ...
    /// Construction-only relation for bindings whose final type is determined
    /// by a pending semantic operation (for example `for` items and match
    /// payloads). `Binding.ty` stays compact/non-optional; consumers must use
    /// this relation to distinguish its temporary compatibility payload from a
    /// real language type.
