...
/// Mechanical finalization of already-linked modules. This is intentionally
/// distinct from GlobalSema: external references, pending operations and
/// generic/abstract templates must be consumed before this function is called.
pub fn globalize(allocator: std.mem.Allocator, modules: []const module_sg.ModuleSemanticGraph) !global_sg.GlobalSemanticGraph {
    ...
    for (modules) |*module| {
        ...
        try requireFinalizable(module);
        try appendModule(allocator, &result, module);
    }

    try global_verify.verifyGlobal(&result);
    return result;
}
...
fn appendModule(allocator: std.mem.Allocator, result: *global_sg.GlobalSemanticGraph, module: *const module_sg.ModuleSemanticGraph) !void {
    ...
    for (module.file_offsets.items) |file| try result.files.append(allocator, .{
        .module = module_id,
        .path = try relocateString(module, file.path, offsets.string_base),
    });
    ...
    try result.modules.append(allocator, .{
        .dir = module_dir,
        .files = .{ .start = offsets.file_base, .len = try index32(module.file_offsets.items.len) },
...
fn globalDecl(o: Offsets, id: module_entities.ModuleDeclId) global_sg.GlobalDeclId { return @enumFromInt(o.declaration_base + @intFromEnum(id)); }
fn globalType(o: Offsets, id: module_entities.ModuleTypeId) global_sg.GlobalTypeId { return @enumFromInt(o.type_base + @intFromEnum(id)); }
fn globalFunction(o: Offsets, id: module_entities.ModuleFunctionId) global_sg.GlobalFunctionId { return @enumFromInt(o.function_base + @intFromEnum(id)); }
fn globalBinding(o: Offsets, id: module_entities.ModuleBindingId) global_sg.GlobalBindingId { return @enumFromInt(o.binding_base + @intFromEnum(id)); }
fn globalNode(o: Offsets, id: module_entities.ModuleNodeId) global_sg.GlobalNodeId { return @enumFromInt(o.node_base + @intFromEnum(id)); }
fn globalBlock(o: Offsets, id: module_entities.ModuleBlockId) global_sg.GlobalBlockId { return @enumFromInt(o.block_base + @intFromEnum(id)); }
fn globalVariant(o: Offsets, id: module_entities.ModuleVariantId) global_sg.GlobalVariantId { return @enumFromInt(o.variant_base + @intFromEnum(id)); }
...
test "globalize preserves canonical declaration layouts and body ids" {
    ...
    const type_name = try strings.append(&module.strings, allocator, "Thing");
    const binding_name = try strings.append(&module.strings, allocator, "x");
    ...
    try module.semantic.declaration_semantics.append(allocator, .{
    ...
    try std.testing.expectEqual(primitives.StructLayout.c_union, global.declarations.items[0].struct_layout);
    try std.testing.expectEqual(@as(u32, 0), @intFromEnum(global.nodes.items[0].content.binding_use));
    try std.testing.expectEqualStrings("Thing", global.text(global.declarations.items[0].name));
}

test "globalize rejects well formed unresolved module types" {
    const allocator = std.testing.allocator;
    var module: module_sg.ModuleSemanticGraph = .{ .module_dir = try allocator.dupe(u8, "demo") };
    defer module.deinit(allocator);
    ...
    const strings = @import("semantic_strings.zig");
    const path = try strings.append(&module.strings, allocator, "main.rg");
    const name = try strings.append(&module.strings, allocator, "Other");
    try module.file_offsets.append(allocator, .{
        .path = path,
        .declaration_base = 0,
        .declaration_count = 0,
    ...
    try module.semantic.external_types.append(allocator, @enumFromInt(0));
    module.semantic.local_semantics_complete = true;

    try std.testing.expectError(error.UnresolvedModuleSemantics, globalize(allocator, &.{module}));
}