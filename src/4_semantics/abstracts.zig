...
fn evalComptimeIntPattern(file: *const syn.SyntaxFile, node: syn.NodeIndex, params: []const gen.GenericParam, bindings: *TemplateBindings) ?i64 {
    return switch (file.nodeTag(node)) {
        .literal => parseIntLiteralToken(file, node),
        .identifier => blk: {
            const name = file.tokenText(file.nodeMainToken(node));
        ...
        .binary_add, .binary_subtract, .binary_multiply, .binary_divide, .binary_modulo => blk: {
            const operands = file.fullBinary(node).?;
            const left = evalComptimeIntPattern(file, operands.lhs, params, bindings) orelse break :blk null;
            const right = evalComptimeIntPattern(file, operands.rhs, params, bindings) orelse break :blk null;
            break :blk switch (file.nodeTag(node)) {
...
fn parseIntLiteralToken(file: *const syn.SyntaxFile, node: syn.NodeIndex) ?i64 {
    return std.fmt.parseInt(i64, file.tokenText(file.nodeMainToken(node)), 0) catch null;
}
...
fn matchComptimeIntPattern(
    ...
    if (file.nodeTag(node) == .identifier) {
        const name = file.tokenText(file.nodeMainToken(node));
        if (findGenericParam(params, name)) |param| {
            if (param.kind == .comptime_int) {
    ...
    const expected = evalComptimeIntPattern(file, node, params, bindings) orelse return false;
    return expected == actual;
}

fn matchTemplateType(files: []const syn.SyntaxFile, pattern: syn.SyntaxRef, actual: sg.Type, params: []const gen.GenericParam, bindings: *TemplateBindings) bool {
    const file = syn.fileForRef(files, pattern);
    return switch (file.fullType(pattern.node) orelse return false) {
        .name => |type_name| blk: {
            const name = file.tokenText(type_name.name_token);
        ...
        .inferred_errable => |inner| blk: {
            ...
            for (choice.variants) |variant| {
                ...
                break :blk matchTemplateType(files, file.ref(inner), payload, params, bindings);
            }
            break :blk false;
        },
        .pointer => |ptr_info| blk: {
            ...
            break :blk matchTemplateType(files, file.ref(ptr_info.child), actual.pointer_type.child.*, params, bindings);
        },
        .array => |arr_info| blk: {
            if (actual != .array_type) break :blk false;
            const length = std.fmt.parseInt(usize, file.tokenText(arr_info.length_token), 10) catch break :blk false;
            if (length != actual.array_type.length) break :blk false;
            break :blk matchTemplateType(files, file.ref(arr_info.element), actual.array_type.element_type.*, params, bindings);
        },
        .struct_literal, .choice_literal, .nullable => false,
        .generic => |g| matchGenericInstantiationType(files, file, g, actual, params, bindings),
    };
}
...
fn matchCanonicalGenericInstantiation(
    ...
    file: *const syn.SyntaxFile,
    g: syn.FullGenericType,
    actual: sg.Type,
    params: []const gen.GenericParam,
    bindings: *TemplateBindings,
) bool {
    const identity = typ.genericIdentityOf(actual) orelse return false;
    const base = file.fullType(g.base) orelse return false;
    const base_name = switch (base) {
        .name => |name| file.tokenText(name.name_token),
        else => return false,
    };
    if (!std.mem.eql(u8, identity.base_name, base_name)) return false;

    const fields = file.fullStructType(g.arguments) orelse return false;
    for (fields.fields) |field_node| {
        const field = file.fullField(field_node) orelse return false;
        const field_name = file.tokenText(field.name_token);
        ...
        switch (arg_value) {
            .type => |arg_ty| {
                ...
                if (!matchTemplateType(files, file.ref(field_ty), arg_ty, params, bindings)) return false;
            },
            .comptime_int => |arg_int| {
                const value_node = field.default_value orelse return false;
                if (!matchComptimeIntPattern(file, value_node, arg_int, params, bindings)) return false;
            },
        }
    }
...
fn matchGenericInstantiationType(
    ...
    return matchCanonicalGenericInstantiation(files, file, g, actual, params, bindings);
}
...
pub fn matchAbstractImplTemplate(tmpl: AbstractImplTemplate, candidate: sg.Type, allocator: *const std.mem.Allocator) ?TemplateBindings {
    ...
    if (!matchTemplateType(tmpl.syntax_files, tmpl.ty, candidate, tmpl.params, &bindings)) {
        bindings.deinit();
        return null;
    }
    return bindings;
    ...
    while (i < req_in.fields.len) : (i += 1) {
        ...
        if (rq.input_nested_patterns.len > i) {
            if (rq.input_nested_patterns[i]) |pattern| {
                if (!matchTemplateType(rq.syntax_files, pattern, cf.ty, &.{}, &nested_bindings)) return false;
                continue;
            }
        }
    ...
    while (i < rq.output.fields.len) : (i += 1) {
        ...
        if (rq.output_nested_patterns.len > i) {
            if (rq.output_nested_patterns[i]) |pattern| {
                if (!matchTemplateType(rq.syntax_files, pattern, co.ty, &.{}, &nested_bindings)) return false;
                continue;
            }
        }
...
fn genericTemplateFieldsMatchExpected(
    ...
    for (template_fields[expected.fields.len..]) |field_node| {
        if (file.fullField(field_node).?.default_value == null) return false;
    }

    var i: usize = 0;
    while (i < expected.fields.len) : (i += 1) {
        const template_field = file.fullField(template_fields[i]) orelse return false;
        const template_ty = template_field.type_node orelse return false;
        if (!matchTemplateType(files, file.ref(template_ty), expected.fields[i].ty, params, bindings)) return false;
    }
...
fn abstractPatternMatchesTemplate(
    ...
    const requirement_type = requirement_file.fullType(requirement.node) orelse return false;
    const candidate_type = candidate_file.fullType(candidate.node) orelse return false;

    if (requirement_type == .name) {
        const requirement_name = requirement_file.tokenText(requirement_type.name.name_token);
        if (std.mem.eql(u8, requirement_name, "Self"))
            return matchTemplateType(files, candidate, concrete, template_params, bindings);
    ...
    return switch (requirement_type) {
        .name => |required_name| candidate_type == .name and
            std.mem.eql(
                u8,
                requirement_file.tokenText(required_name.name_token),
                candidate_file.tokenText(candidate_type.name.name_token),
            ),
        .pointer => |required_pointer| candidate_type == .pointer and
            required_pointer.mutability == candidate_type.pointer.mutability and
            abstractPatternMatchesTemplate(files, requirement_file.ref(required_pointer.child), candidate_file.ref(candidate_type.pointer.child), concrete, abstract_param_names, template_params, bindings),
        .array => |required_array| blk: {
            if (candidate_type != .array) break :blk false;
            const required_length = std.fmt.parseInt(u64, requirement_file.tokenText(required_array.length_token), 10) catch break :blk false;
            const candidate_length = std.fmt.parseInt(u64, candidate_file.tokenText(candidate_type.array.length_token), 10) catch break :blk false;
        ...
        .generic => |required_generic| blk: {
            ...
            const required_base = requirement_file.fullType(required_generic.base) orelse break :blk false;
            const candidate_base = candidate_file.fullType(candidate_generic.base) orelse break :blk false;
            if (required_base != .name or candidate_base != .name) break :blk false;
            if (!std.mem.eql(u8, requirement_file.tokenText(required_base.name.name_token), candidate_file.tokenText(candidate_base.name.name_token))) break :blk false;
            const required_fields = requirement_file.fullStructType(required_generic.arguments).?.fields;
            const candidate_fields = candidate_file.fullStructType(candidate_generic.arguments).?.fields;
            if (required_fields.len != candidate_fields.len) break :blk false;
            for (required_fields) |required_field_node| {
                const required_field = requirement_file.fullField(required_field_node).?;
                const required_name = requirement_file.tokenText(required_field.name_token);
                var candidate_field: ?syn.FullField = null;
                for (candidate_fields) |field_node| {
                    const field = candidate_file.fullField(field_node).?;
                    if (std.mem.eql(u8, required_name, candidate_file.tokenText(field.name_token))) {
                ...
                if (required_field.type_node) |required_type| {
                    ...
                    if (!abstractPatternMatchesTemplate(files, requirement_file.ref(required_type), candidate_file.ref(actual_type), concrete, abstract_param_names, template_params, bindings)) break :blk false;
                }
            }
            break :blk true;
        },
        .inferred_errable => |required_inner| candidate_type == .inferred_errable and
            abstractPatternMatchesTemplate(files, requirement_file.ref(required_inner), candidate_file.ref(candidate_type.inferred_errable), concrete, abstract_param_names, template_params, bindings),
        .struct_literal, .choice_literal, .nullable => false,
    };
}
...
fn genericTemplateFieldsMatchRequirement(
    ...
    for (template_fields[expected.len..]) |field_node| if (file.fullField(field_node).?.default_value == null) return false;
    var matched_nested = false;
    for (expected, 0..) |_, index| {
        if (nested.len > index and nested[index] != null) {
            matched_nested = true;
            const candidate_type = file.fullField(template_fields[index]).?.type_node orelse return false;
            if (!abstractPatternMatchesTemplate(rq.syntax_files, nested[index].?, file.ref(candidate_type), concrete, rq.abstract_param_names, template_params, bindings)) return false;
        }
    }
    ...
    while (cur) |sc| : (cur = sc.parent) {
        ...
        if (sc.generic_functions.getPtr(rq.name)) |lst| {
            ...
            for (lst.items) |tmpl| {
                ...
                const input_fields = syntax_file.fullStructType(tmpl.input).?.fields;
                const output_fields = syntax_file.fullStructType(tmpl.output).?.fields;

                if (!genericTemplateFieldsMatchExpected(tmpl.syntax_files, syntax_file, expected_in, input_fields, tmpl.params, &bindings))
                    continue;
                if (!genericTemplateFieldsMatchRequirement(syntax_file, &rq, true, input_fields, concrete, tmpl.params, &bindings))
                    continue;
                if (!genericTemplateFieldsMatchExpected(tmpl.syntax_files, syntax_file, expected_out, output_fields, tmpl.params, &bindings) and
