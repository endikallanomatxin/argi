...
pub const MemorySafetyAnalyzer = struct {
    ...
    fn widenTemporalSummary(self: *MemorySafetyAnalyzer, function: *const sg.FunctionDeclaration) !void {
        ...
        for (function.input.fields, 0..) |input, input_index| {
            ...
            if (input_type != .pointer_type or input_type.pointer_type.mutability != .cleanup) continue;
    ...
    fn analyzeNode(self: *MemorySafetyAnalyzer, node: *const sg.SGNode, state: *FunctionState) anyerror!void {
        switch (node.content) {
            ...
            .virtual_call => |virtual_call| {
                ...
                // Virtual dispatch hides the concrete body. A writable Self
                // method may end roots reachable through Self, so callers must
                // conservatively treat existing references as invalidated.
                if (typ.pointerCanWrite(virtual_call.self_permission)) {
            ...
            .pointer_assignment => |assignment| {
                ...
                if (assignment.pointer.sem_type) |pointer_ty| {
                    if (pointer_ty == .pointer_type and pointer_ty.pointer_type.mutability == .cleanup) {
    ...
    /// Until a precise summary is available, an cleanup parameter may
    ...
    fn recordConservativeCallInvalidations(
        ...
        for (input.fields, 0..) |field, index| {
            ...
            if (parameter.ty != .pointer_type or parameter.ty.pointer_type.mutability != .cleanup) continue;
    ...
    /// The cleanup reference used to perform a whole-referent transition
        ...
        for (function.temporal_contract.invalidates_inputs) |input_index| {
            try invalidations.append(.{
                ...
                .refreshes_input = input_index < function.input_bindings.len and
                    ...
                    function.input_bindings[input_index].ty.pointer_type.mutability == .cleanup,
            });
        }
        for (function.temporal_contract.invalidates_dependencies) |contract_invalidation| {
            ...
            if (resolved.input_index < function.input_bindings.len) {
                ...
                resolved.refreshes_input = input_type == .pointer_type and
                    input_type.pointer_type.mutability == .cleanup;
            }
        ...
        for (state.invalidations.items) |invalidation| {
            ...
            try invalidations.append(.{
                ...
                .refreshes_input = source.input_index < function.input_bindings.len and
                    ...
                    function.input_bindings[source.input_index].ty.pointer_type.mutability == .cleanup,
            });
        }
        // An cleanup input without an explicit or inferable footprint keeps
