...
pub fn verifyModule(graph: *const graph_mod.ModuleSemanticGraph) !void {
    ...
    const field_count = graph.fields.items.len + graph.structural_fields.items.len;
    const variant_count = graph.choice_variant_entries.items.len + graph.structural_choice_variants.items.len;

    for (graph.declarations.items) |decl| {
        try require(verify.stringFits(decl.name, graph.strings.items));
        try require(decl.module_file_index < graph.file_offsets.items.len);
        ...
        if (decl.struct_fields) |range| try require(verify.rangeFits(range, field_count));
        if (decl.choice_variants) |range| try require(verify.rangeFits(range, variant_count));
    }
    ...
    for (graph.type_references.items) |reference| {
        ...
        try require(verify.optionalIdFits(reference.resolved_type, graph.types.items.len));
    ...
    for (graph.types.items, 0..) |_, index| try payload.semanticType(entities.Ids, try views.typeView(graph, @enumFromInt(@as(u32, @intCast(index)))), bounds);
    for (0..field_count) |index| try payload.field(entities.Ids, try views.fieldView(graph, @enumFromInt(@as(u32, @intCast(index)))), bounds);
    for (0..variant_count) |index| try payload.variant(entities.Ids, try views.variantView(graph, @enumFromInt(@as(u32, @intCast(index)))), bounds);
    for (graph.generic_type_arguments.items, 0..) |_, index| try payload.genericArgument(entities.Ids, try views.genericArgumentView(graph, @enumFromInt(@as(u32, @intCast(index)))), bounds);
    for (graph.functions.items, 0..) |_, index| try payload.function(entities.Ids, try views.functionView(graph, @enumFromInt(@as(u32, @intCast(index)))), bounds);
    ...
    for (semantic.type_refs.items) |id| try require(verify.idFits(id, graph.types.items.len));
...
fn verifyOverlays(graph: *const graph_mod.ModuleSemanticGraph) !void {
    for (graph.semantic.function_semantics.items, 0..) |value, index| {
        ...
        try require(verify.optionalIdFits(value.inferred_error_reasons, graph.types.items.len));
    }
    const field_count = graph.fields.items.len + graph.structural_fields.items.len;
    for (graph.semantic.field_semantics.items, 0..) |value, index| {
        try require(verify.idFits(value.field, field_count));
        try require(!hasFieldOverlayBefore(graph, index, value.field));
        try require(verify.optionalIdFits(value.storage_type, graph.types.items.len));
    ...
    for (graph.semantic.variant_semantics.items, 0..) |value, index| {
        try require(verify.idFits(value.variant, variant_count));
...
fn verifyPending(graph: *const graph_mod.ModuleSemanticGraph, operation: entities.PendingOperation) !void {
    ...
    switch (operation) {
        .resolve_type => |value| {
            ...
            try require(verify.idFits(value.destination, graph.types.items.len));
        },
        .resolve_call => |value| {
...
fn makeBounds(graph: *const graph_mod.ModuleSemanticGraph) payload.Bounds {
    ...
    return .{
        ...
        .fields = graph.fields.items.len + graph.structural_fields.items.len,
        .variants = graph.choice_variant_entries.items.len + graph.structural_choice_variants.items.len,
